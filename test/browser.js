...
const URL = 'chrome-extension://fggohbjlmjldccoelobkepngbkkebefi/src/index.html?view=options'
...
async function getObserver () {
  try {
    return await page.evaluate(async (config) => {
      return JSON.stringify(window.observer)
    }, config)
  } catch (e) {
    console.error(e)
  }
...
describe('Setup', () => {
  before(async function () {
    ...
    userdir = await fs.mkdtemp(path.join(os.tmpdir(), 'guld-chrome-test'))
    // copy the ledger-native bindings to our temp dir
    const P = `/NativeMessagingHosts/com.guld.ledger.json`
    await fs.mkdirp(path.dirname(`${userdir}${P}`))
    await fs.copyFile(`${HOME}/.config/chromium${P}`, `${userdir}${P}`)
  ...
  describe('Initialization', () => {
    ...
    it('create Observer', async () => {
      var o = await getObserver()
      console.log(o)
      assert.exists(o)
    })
